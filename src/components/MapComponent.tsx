...
export default function MapComponent({
  ...
  useEffect(() => {
    ...
    // Center on Dehradun, Uttarakhand, India (Dehradun: 30.3165, 78.0322)
      ...
      if (existingLayer) {
        ...
        try {
          ...
          existingLayer.setStyle({
            ...
            fill: !isLine, // Ensure line layers do not have a filled region
            fillOpacity: isPolygon ? 0.0 : (isLine ? 0.0 : (layerConf.fillOpacity * layerConf.opacity)),
          });
        } catch (styleErr) {
        ...
        const geoJsonLayer = L.geoJSON(geoJsonData, {
          ...
          style: (feature: any) => {
            ...
            return {
              ...
              weight: layerConf.weight,
              opacity: layerConf.opacity,
              fill: !isLine, // Line geometries should not be filled
              fillOpacity: isPolygon ? 0.0 : (isLine ? 0.0 : (layerConf.fillOpacity * layerConf.opacity)),
            };
          },
  ...
  // Map Controls: Pan to Dehradun bounds safely
  const handleZoomToDistrict = () => {
    ...
    // Fallback static center of Dehradun
    map.setView([30.3165, 78.0322], 9, { animate: true });
  };
  ...
  return (
    <div className="relative flex-1 bg-slate-100 flex flex-col h-full min-w-0">
      ...
      <div className="absolute bottom-4 left-4 z-[1000] bg-white/90 dark:bg-slate-900/90 backdrop-blur border border-slate-200 dark:border-slate-800 shadow-xs px-3 py-1.5 rounded-md flex items-center gap-4 text-xs font-mono text-slate-600 dark:text-slate-300 transition-colors">
        <div className="flex items-center gap-1.5">
          <Move className="w-3.5 h-3.5 text-slate-400 dark:text-slate-500" />
          <span>X (Lng): <strong ref={lngRef} className="text-slate-800 dark:text-slate-200">---</strong></span>
          <span className="text-slate-300 dark:text-slate-700">|</span>
          <span>Y (Lat): <strong ref={latRef} className="text-slate-800 dark:text-slate-200">---</strong></span>
        </div>
      ...
      <div className="absolute top-4 left-4 z-[1001] w-80 font-sans">
        <div className="relative flex items-center bg-white/95 dark:bg-slate-900/95 backdrop-blur border border-slate-200 dark:border-slate-800 rounded-lg shadow-md transition-shadow duration-200 focus-within:shadow-lg focus-within:border-indigo-400">
              ...
              return (
                ...
                >
                  ...
                  <span className="text-[9px] uppercase tracking-wider bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500 font-bold px-1.5 py-0.5 rounded font-mono shrink-0">
                    {layerName.replace("USN-", "").replace("Almora-", "").replace("Dehradun-", "").replace("-Boundary", "")}
                  </span>
                </button>
              );
            })}
